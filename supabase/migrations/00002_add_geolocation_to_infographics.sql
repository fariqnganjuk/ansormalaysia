-- Add geolocation data to infographics table
ALTER TABLE infographics ADD COLUMN IF NOT EXISTS location_name TEXT;
ALTER TABLE infographics ADD COLUMN IF NOT EXISTS latitude DECIMAL(10, 8);
ALTER TABLE infographics ADD COLUMN IF NOT EXISTS longitude DECIMAL(11, 8);
ALTER TABLE infographics ADD COLUMN IF NOT EXISTS data_value INTEGER;
ALTER TABLE infographics ADD COLUMN IF NOT EXISTS data_type TEXT;

-- Add index for location queries
CREATE INDEX IF NOT EXISTS idx_infographics_location ON infographics(location_name);

-- Insert sample geolocation data for Malaysian states with PMI statistics
INSERT INTO infographics (title, description, image_url, location_name, latitude, longitude, data_value, data_type) VALUES
